...
# Transactional MediaBot v2.8.0 deployment for MediaServer.
#
# Usage (run as root on MediaServer):
#   MEDIABOT_ALLOWED_GUILD_IDS="123456789012345678" \
#       /path/to/deploy_v280.sh /tmp/mediabot-v280-<unique-id>
#
# The source archive remains unchanged. deploy/compose.homelab.yaml is selected
# explicitly and installed as the live compose.yaml; portable storage is never used.
...
stage_namespace="/tmp/mediabot-v280-"
target="/opt/stacks/mediabot"
container="mediabot"
service="mediabot"
lock_dir="/var/lock/mediabot-deploy.lock"
legacy_lock_dir="/var/lock/mediabot-v09-deploy.lock"
legacy_manifest_files="app.py Dockerfile compose.yaml requirements.txt .env.example .dockerignore"
manifest_files="$legacy_manifest_files LICENSE"
life_inbox="/opt/stacks/life/vault/Life/Inbox"
torrent_intake_network="dogginator_torrent_intake"
torrent_intake_token="/opt/stacks/torrent/secrets/torrent_intake_token"
review_settings="/opt/stacks/torrent/.env"
...
restore_runtime_manifest() {
    run_target_helper sh -eu -c '
        ...
        if test -f "$root/runtime/LICENSE"; then
            temporary="/target/.LICENSE.restore-$DEPLOY_STAMP"
            cp -a "$root/runtime/LICENSE" "$temporary"
            mv -f "$temporary" /target/LICENSE
        else
            # This release introduced LICENSE; restore its recorded prior absence.
            rm -f -- /target/LICENSE
        fi
        ...
        for name in app.py Dockerfile compose.yaml requirements.txt .env.example .dockerignore LICENSE; do
...
on_exit() {
    ...
    if test "$legacy_lock_held" -eq 1; then
        rmdir "$legacy_lock_dir" >/dev/null 2>&1 || :
    fi
...
say "1. Validate v2.8.0 stage, deployment input, and host prerequisites"
...
case "$stage" in
    "$stage_namespace"*) ;;
    *) die "Resolved stage escaped the v2.8.0 staging namespace." ;;
esac
...
lock_held=1
mkdir "$legacy_lock_dir" 2>/dev/null \
    || die "A legacy v0.9 deployment is active, or the stale lock $legacy_lock_dir needs inspection."
legacy_lock_held=1

stamp="$(date -u +%Y%m%dT%H%M%SZ)-$$"
backup_rel=".codex-backups/${stamp}-v280"
backup="$target/$backup_rel"
candidate_image="mediabot-v280-candidate:${stamp}"
rollback_image="mediabot-rollback:pre-v280-${stamp}"
release_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"

for name in $manifest_files; do
    require_regular_file "$stage/$name"
done
for name in $legacy_manifest_files; do
    require_regular_file "$target/$name"
done
if test -e "$target/LICENSE" || test -L "$target/LICENSE"; then
    require_regular_file "$target/LICENSE"
fi
homelab_compose="$stage/deploy/compose.homelab.yaml"
require_regular_file "$homelab_compose"
require_regular_file "$stage/scripts/homelab_deployment.py"
require_regular_file "$stage/SOURCE.sha256"
(cd "$stage" && sha256sum -c SOURCE.sha256 >/dev/null) \
    || die "Immutable release source manifest did not verify."
...
require_regular_file "$torrent_intake_token"
require_regular_file "$review_settings"
review_owner="$(sed -n 's/^REVIEW_OWNER_IDS=//p' "$review_settings" | tail -n 1 | cut -d, -f1)"
case "$review_owner" in
    ''|*[!0-9]*) die "The trusted review service needs a configured owner before this release." ;;
esac
...
test -d "$life_inbox" && test ! -L "$life_inbox" \
    || die "Private Life inbox is missing or unsafe."
test "$(readlink -f -- "$life_inbox")" = "$life_inbox" \
    || die "Private Life inbox resolves through an unexpected symbolic link."
test "$(readlink -f -- "$torrent_intake_token")" = "$torrent_intake_token" \
    || die "Torrent intake token resolves through an unexpected symbolic link."
test "$(stat -c '%a' "$torrent_intake_token")" = "600" \
    || die "Torrent intake token must have mode 600."
...
docker network inspect "$torrent_intake_network" >/dev/null 2>&1 \
    || die "Internal torrent intake network is missing."
test "$(docker network inspect -f '{{.Internal}}' "$torrent_intake_network")" = "true" \
    || die "Torrent intake network is not Docker-internal."

for network in life_intake local-ai_frontend mediabot_search_frontend; do
    docker network inspect "$network" >/dev/null 2>&1 || die "Required gateway network is missing: $network"
done
test "$(docker network inspect -f '{{.Internal}}' mediabot_search_frontend)" = "true" || die "Search frontend network is not Docker-internal."
test "$(docker network inspect -f '{{.Internal}}' life_intake)" = "true" \
    || die "Life intake network is not Docker-internal."
for credential in /opt/stacks/life-workflows/secrets/life_gateway_token /opt/stacks/local-ai/secrets/local_ai_token; do
    require_regular_file "$credential"
    test "$(readlink -f -- "$credential")" = "$credential" || die "Gateway credential path is unsafe."
    test "$(stat -c '%u:%g %a' "$credential")" = "1000:1000 600" || die "Gateway credential permissions are unsafe."
done
for gateway in life-gateway local-ai-gateway; do
    test "$(docker inspect -f '{{.State.Health.Status}}' "$gateway")" = "healthy" \
        || die "Required gateway is not healthy: $gateway"
done
...
say "2. Compile, test, and build the exact staged v2.8.0 release"
...
docker run --rm --network none --read-only --tmpfs /tmp:size=16m \
    --user 1000:1000 \
    -v "$life_inbox:/life-vault/Life/Inbox" \
    --entrypoint python "$candidate_image" -c '
import os
path = "/life-vault/Life/Inbox/.mediabot-capture-write-probe"
...
try:
    ...
    os.close(descriptor)
os.unlink(path)
print("LIFE_INBOX_WRITE ok")
'

docker run --rm --network "$torrent_intake_network" --read-only \
    --tmpfs /tmp:size=32m --cap-drop ALL --security-opt no-new-privileges \
    --user 1000:1000 \
    -e TORRENT_INTAKE_URL=http://torrent-intake:8765 \
    -e TORRENT_INTAKE_TOKEN_PATH=/run/secrets/torrent_intake_token \
    -e "TORRENT_REVIEW_OWNER_ID=$review_owner" \
    -v "$torrent_intake_token:/run/secrets/torrent_intake_token:ro" \
    --entrypoint python "$candidate_image" -c '
import asyncio, os
from mediabot.services.torrent_intake import TorrentIntakeService
async def main():
    service = TorrentIntakeService()
    await service.start()
    try:
        await service.health()
        await service.review_request("list", actor_id=int(os.environ["TORRENT_REVIEW_OWNER_ID"]), actor_role="owner")
    finally:
        await service.close()
asyncio.run(main())
print("TORRENT_INTAKE ok")
'
...
run_target_helper sh -eu -c '
    ...
    if test -f /target/LICENSE; then
        cp -a /target/LICENSE "$root/runtime/LICENSE"
    fi
...
docker run --rm --network none --read-only --tmpfs /tmp:size=32m \
    ...
    "$helper_image" sh -eu -c '
        ...
        for name in app.py Dockerfile compose.yaml requirements.txt .env.example .dockerignore LICENSE; do
            ...
            source="/stage/$name"
            if test "$name" = compose.yaml; then
                source=/stage/deploy/compose.homelab.yaml
            fi
...
for name in $manifest_files; do
    source="$stage/$name"
    if test "$name" = compose.yaml; then
        source="$homelab_compose"
    fi
    cmp -s "$source" "$target/$name" || die "Installed manifest mismatch: $name"
done
(cd "$stage" && sha256sum -c SOURCE.sha256 >/dev/null) \
    || die "Release source changed during installation."
cp "$stage/SOURCE.sha256" "$backup/release-source.sha256"
sha256sum "$homelab_compose" > "$backup/selected-homelab-compose.sha256"
chmod 600 "$backup/release-source.sha256" "$backup/selected-homelab-compose.sha256"
...
import os, sqlite3
root = "/app/data"
probe = os.path.join(root, ".mediabot-write-probe-v280")
...
say "7. Build and recreate MediaBot v2.8.0"
(cd "$target" && docker compose --project-directory "$target" -f "$target/compose.yaml" up -d --build --force-recreate "$service")
bot_stopped=0
...
if test "$rollback_drill" -eq 1; then
    say "Rollback drill reached a healthy v2.8.0 candidate; forcing guarded rollback."
...
docker exec "$container" python -c '
import sqlite3
from mediabot.core.event_store import EVENT_SCHEMA_VERSION, EventStore
...
try:
    ...
    required = {
        ...
        "event_time_options", "event_time_votes", "event_reminder_state",
    }
    assert required <= tables
    request_columns = {
        row[1] for row in connection.execute("PRAGMA table_info(request_messages)")
    }
    assert {
        "availability_notification_message_id",
        "availability_notified_at",
    } <= request_columns
    assert connection.execute("SELECT MAX(version) FROM event_schema_migrations").fetchone()[0] == EVENT_SCHEMA_VERSION
    EventStore._validate_v2(connection)
finally:
    connection.close()
print("DATABASE ok")
'
...
async def main():
    ...
    if app.life_capture is None:
        raise RuntimeError("private capture inbox is disabled")
    probe = app.life_capture.inbox_path / ".mediabot-release-write-probe"
    ...
    try:
        ...
        os.close(descriptor)
    os.unlink(probe)
    await app.torrent_intake.start()
    try:
        await app.torrent_intake.health()
    finally:
        await app.torrent_intake.close()
try:
    asyncio.run(main())
    ...
    print("PROVIDERS_LIFE_AND_TORRENT_INTAKE ok")
'

docker exec -e "RELEASE_OWNER=$review_owner" "$container" python -c '
import asyncio, os, app
async def main():
    assert app.life_workflow.base_url == "http://life-gateway:8091"
    assert app.local_ai.base_url == "http://local-ai-gateway:8080"
    owner = int(os.environ["RELEASE_OWNER"])
    try:
        await app.life_workflow.request("captures", actor_id=owner, limit=1)
        await app.life_workflow.request("tasks", actor_id=owner, limit=1)
        await app.local_ai.start()
        async with app.local_ai.session.get(app.local_ai.base_url + "/v1/status", allow_redirects=False) as response:
            assert response.status == 200
            body = await response.json()
            assert body.get("retrieval_enabled") is False
            assert body.get("tools_enabled") is False
    finally:
        await app.life_workflow.close()
        await app.local_ai.close()
asyncio.run(main())
print("LIFE_AND_LOCAL_AI_GATEWAYS ok")
'
docker inspect "$container" | docker run --rm -i --network none --read-only "$helper_image" python -c '
import json, sys
state = json.load(sys.stdin)[0]
networks = set(state["NetworkSettings"]["Networks"])
assert {"dogginator_torrent_intake", "life_intake", "local-ai_frontend"} <= networks
assert not networks.intersection({"local-ai_private", "nextcloud_backend", "nextcloud_frontend"})
mounts = {item["Destination"]: item for item in state["Mounts"]}
for name in ("life_gateway_token", "local_ai_token"):
    assert not mounts["/run/secrets/" + name]["RW"]
assert not any("docker.sock" in item["Destination"] for item in state["Mounts"])
print("GATEWAY_NETWORK_AND_SECRET_BOUNDARIES ok")
'
...
print(digest.hexdigest())
')"
test "$source_digest" = "$container_digest" || die "Running source does not match staged v2.8.0 source."
...
log_probe="$(mktemp /tmp/mediabot-v280-logs.XXXXXX)"
chmod 600 "$log_probe"
...
if test "$legacy_lock_held" -eq 1; then
    rmdir "$legacy_lock_dir" || die "Deployment succeeded but the legacy lock could not be removed: $legacy_lock_dir"
    legacy_lock_held=0
fi
if test "$lock_held" -eq 1; then
...
say "10. MediaBot v2.8.0 deployment passed every release gate"
