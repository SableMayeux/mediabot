...
# Transactional MediaBot v2.1.3 deployment for MediaServer.
#
# Usage (run as root on MediaServer):
#   MEDIABOT_ALLOWED_GUILD_IDS="123456789012345678" \
#       /path/to/deploy_v213.sh /tmp/mediabot-v213-<unique-id>
#
...
stage_namespace="/tmp/mediabot-v213-"
...
say "1. Validate v2.1.3 stage, deployment input, and host prerequisites"
...
case "$stage" in
    "$stage_namespace"*) ;;
    *) die "Resolved stage escaped the v2.1.3 staging namespace." ;;
esac
...
backup_rel=".codex-backups/${stamp}-v213"
backup="$target/$backup_rel"
candidate_image="mediabot-v213-candidate:${stamp}"
rollback_image="mediabot-rollback:pre-v213-${stamp}"
...
say "2. Compile, test, and build the exact staged v2.1.3 release"
...
import os, sqlite3
root = "/app/data"
probe = os.path.join(root, ".mediabot-write-probe-v213")
...
say "7. Build and recreate MediaBot v2.1.3"
(cd "$target" && docker compose up -d --build --force-recreate "$service")
bot_stopped=0
...
if test "$rollback_drill" -eq 1; then
    say "Rollback drill reached a healthy v2.1.3 candidate; forcing guarded rollback."
...
print(digest.hexdigest())
')"
test "$source_digest" = "$container_digest" || die "Running source does not match staged v2.1.3 source."
...
log_probe="$(mktemp /tmp/mediabot-v213-logs.XXXXXX)"
chmod 600 "$log_probe"
...
say "10. MediaBot v2.1.3 deployment passed every release gate"
