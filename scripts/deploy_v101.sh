...
# Transactional MediaBot v1.0.1 deployment for MediaServer.
#
# Usage (run as root on MediaServer):
#   MEDIABOT_ALLOWED_GUILD_IDS="123456789012345678" \
#       /path/to/deploy_v101.sh /tmp/mediabot-v101-<unique-id>
#
...
stage_namespace="/tmp/mediabot-v101-"
...
say "1. Validate v1.0.1 stage, deployment input, and host prerequisites"
...
case "$stage" in
    "$stage_namespace"*) ;;
    *) die "Resolved stage escaped the v1.0.1 staging namespace." ;;
esac
...
backup_rel=".codex-backups/${stamp}-v101"
backup="$target/$backup_rel"
candidate_image="mediabot-v101-candidate:${stamp}"
rollback_image="mediabot-rollback:pre-v101-${stamp}"
...
say "2. Compile, test, and build the exact staged v1.0.1 release"
...
import os, sqlite3
root = "/app/data"
probe = os.path.join(root, ".mediabot-write-probe-v101")
...
say "7. Build and recreate MediaBot v1.0.1"
(cd "$target" && docker compose up -d --build --force-recreate "$service")
bot_stopped=0
...
if test "$rollback_drill" -eq 1; then
    say "Rollback drill reached a healthy v1.0.1 candidate; forcing guarded rollback."
...
print(digest.hexdigest())
')"
test "$source_digest" = "$container_digest" || die "Running source does not match staged v1.0.1 source."
...
log_probe="$(mktemp /tmp/mediabot-v101-logs.XXXXXX)"
chmod 600 "$log_probe"
...
say "10. MediaBot v1.0.1 deployment passed every release gate"
