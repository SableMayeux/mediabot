...
# Transactional MediaBot v2.7.2 deployment for MediaServer.
#
# Usage (run as root on MediaServer):
#   MEDIABOT_ALLOWED_GUILD_IDS="123456789012345678" \
#       /path/to/deploy_v272.sh /tmp/mediabot-v272-<unique-id>
#
...
stage_namespace="/tmp/mediabot-v272-"
...
say "1. Validate v2.7.2 stage, deployment input, and host prerequisites"
...
case "$stage" in
    "$stage_namespace"*) ;;
    *) die "Resolved stage escaped the v2.7.2 staging namespace." ;;
esac
...
backup_rel=".codex-backups/${stamp}-v272"
backup="$target/$backup_rel"
candidate_image="mediabot-v272-candidate:${stamp}"
rollback_image="mediabot-rollback:pre-v272-${stamp}"
...
for network in life_intake local-ai_frontend; do
...
say "2. Compile, test, and build the exact staged v2.7.2 release"
...
import os, sqlite3
root = "/app/data"
probe = os.path.join(root, ".mediabot-write-probe-v272")
...
say "7. Build and recreate MediaBot v2.7.2"
...
if test "$rollback_drill" -eq 1; then
    say "Rollback drill reached a healthy v2.7.2 candidate; forcing guarded rollback."
...
print(digest.hexdigest())
')"
test "$source_digest" = "$container_digest" || die "Running source does not match staged v2.7.2 source."
docker inspect "$container" | python "$stage/scripts/homelab_deployment.py" --kind container
...
log_probe="$(mktemp /tmp/mediabot-v272-logs.XXXXXX)"
chmod 600 "$log_probe"
...
say "10. MediaBot v2.7.2 deployment passed every release gate"
