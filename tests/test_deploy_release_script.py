...
SCRIPT = ROOT / "scripts" / "deploy_v230.sh"


class DeployV230ContractTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.source = SCRIPT.read_text(encoding="utf-8")
        cls.legacy_source = (ROOT / "scripts" / "deploy_v210.sh").read_text(
            encoding="utf-8"
        )
    ...
    def test_release_and_stage_are_exact(self):
        self.assertIn('release_version="2.3.0"', self.source)
        self.assertIn('stage_namespace="/tmp/mediabot-v230-"', self.source)
    ...
    def test_guild_scope_uid_migration_and_write_probe_are_mandatory(self):
        ...
        self.assertIn(".mediabot-write-probe-v230", self.source)
        self.assertIn(
    ...
    def test_v2_private_boundaries_are_preflighted_and_release_gated(self):
        self.assertIn('life_inbox="/opt/stacks/life/vault/Life/Inbox"', self.source)
        self.assertIn('torrent_intake_network="dogginator_torrent_intake"', self.source)
        self.assertIn("docker network inspect", self.source)
        self.assertIn(
        ...
        self.assertIn(".mediabot-capture-write-probe", self.source)
        self.assertIn("await service.health()", self.source)
        self.assertIn("await app.torrent_intake.health()", self.source)
        self.assertIn("PROVIDERS_LIFE_AND_TORRENT_INTAKE ok", self.source)
    ...
    def test_release_gates_cover_runtime_and_container_hardening(self):
        required = (
            "wait_for_health",
            "runtime_health",
            "PROVIDERS_LIFE_AND_TORRENT_INTAKE ok",
            "DATABASE ok",
            "ReadonlyRootfs",
            "PidsLimit",
            "CapDrop",
